...
void ConnectionHandler::ReportErrorAndExit(char function_name[], int exit_code)
{
	std::cerr << "[ERROR] [" << function_name << "] " << SDLNet_GetError() << '\n'
		<< "Press enter to exit...\n";
	if (exit_code == ErrorCode::RESOLVE_HOST) {
		SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "[SDLNet_ResolveHost]", "Couldn't find remote host!", nullptr);
	}
	else if (exit_code == ErrorCode::RECEIVE) {
		SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "[SDLNet_Receive]", "Error while receiving data!", nullptr);
	}
	else if (exit_code == ErrorCode::SEND) {
		SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "[SDLNet_Send]", "Error while sending data!", nullptr);
	}
	else {
